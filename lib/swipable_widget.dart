...
class _SwipeableWidgetState extends State<SwipeableWidget>
  ...
  late double _initControllerValue;

  @override
  void initState() {
    super.initState();
    _initControllerValue = widget.height / widget.screenSize;
    _controller = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 300),
      value: _initControllerValue,
    )..addListener(() {
        if (_controller.value > _initControllerValue) {
          setState(() {});
          widget.onSwipeStartcallback(
              _controller.value > _initControllerValue + 0.1,
              _controller.value);
        }
        if (_controller.value == _initControllerValue) {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        onPanUpdate: (details) {
          ...
          final minimumXToStartSwiping = widgetSize * 0.15;
...
//          widget.onSwipeStartcallback(_controller.value);
        ...
        onPanEnd: (details) async {
          ...
          if (delta > deltaNeededToBeSwiped) {
            ...
            _controller.animateTo(_initControllerValue,
                duration: const Duration(milliseconds: 300),
                curve: Curves.fastOutSlowIn);
          }
//          widget.onSwipeStartcallback(_controller.value);
